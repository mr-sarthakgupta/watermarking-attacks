...
class CaptionBasedEdit():
    def __init__(self, img_size=256, save_imgs=True):
...
class EditImage():
    def __init__(self, prompt="make it snow", img_guide_scale=1.5, save_imgs=True):
        self.prompt = prompt
        model_id = "timbrooks/instruct-pix2pix"
        self.pipe = StableDiffusionInstructPix2PixPipeline.from_pretrained(model_id, torch_dtype=torch.float16, safety_checker=None)
        self.pipe.to("cuda")
...
class DiffPure():
    def __init__(self, steps=0.4, save_imgs=True, fname="base"):
...
class ImageRephrase():
    def __init__(self, img_size=256, strength=0.5, num_passes=1, save_imgs=True, fname="base"):
        self.image_captioner = ImageCaptioner(image_size=img_size)
...
def get_transforms(aug_str, fname="base", save_images=True):
    aug = aug_str.split(',')[0]
    ...
    if aug == 'no_aug':
        ...
        transform = transforms.Compose(
            [
                transforms.Resize(256),
                EditImage(params[0], float(params[1]), save_imgs=True),
                transforms.ToTensor(),
            ]
        )
        ...
        transform = transforms.Compose([
            transforms.Resize(256),
            ImageRephrase(img_size=256, strength=float(params[0]), num_passes=int(params[1]), save_imgs=True, fname=fname),
            transforms.ToTensor(),
        ])
    elif aug == 'capedit':
        transform = transforms.Compose([
            transforms.Resize(256),
            CaptionBasedEdit(img_size=256, save_imgs=True),
            transforms.ToTensor(),
        ])
    else:
...
def main():
    ...
    parser.add_argument("--save-images", default=True, action='store_true')
    ...
    if args.attack == 'no_aug':
        ...
        # aug_list = ['diffpure_latent,0.1,1', 'diffpure_latent,0.05,1', 'diffpure_latent,0.02,1']
        aug_list = ['diffpure_latent,0.0001,1']
    ...
    for aug in aug_list:
        ...
        transform, org_transform = get_transforms(aug, args.wm_method)
        if transform == None:
            continue
