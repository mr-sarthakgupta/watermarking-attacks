...
def adv_surrogate_model_attack(
    ...
    for i, (images, image_paths) in enumerate(test_loader):
        ...
        adv_preds = model(images_adv)
        for t in range(len(base_preds)):
            print(f"{i + t} | base_pred: {base_preds[t]} | adv_pred: {adv_preds[t]}")
            if torch.argmax(base_preds[t]) != torch.argmax(adv_preds[t]):
                num_success += 1
