...
if __name__ == '__main__':
    ...
    input_dir = "Neurips24_ETI_BeigeBox"
    output_dirs = {
        "regen_diff": "regen_diff_output",
        "rinse_2xDiff": "rinse_2xDiff_output",
        "rinse_4xDiff": "rinse_4xDiff_output",
        "regen_vae": "regen_vae_output",
    }
    ...
    for image_path in tqdm(image_paths):
        ...
        # Apply regen_diff
        output_image = regen_diff(image, strength=60, model=args.vae_name, device=args.device)
        output_image.save(os.path.join(output_dirs["regen_diff"], base_name))

        # Apply rinse_2xDiff
        output_image = rinse_2xDiff(image, strength=60, model=args.vae_name, device=args.device)
        output_image.save(os.path.join(output_dirs["rinse_2xDiff"], base_name))

        # Apply rinse_4xDiff
        output_image = rinse_4xDiff(image, strength=60, model=args.vae_name, device=args.device)
        output_image.save(os.path.join(output_dirs["rinse_4xDiff"], base_name))

        # Apply regen_vae
        output_image = regen_vae(image, strength=60, model=args.vae_name, device=args.device)
        output_image.save(os.path.join(output_dirs["regen_vae"], base_name))